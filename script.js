document.addEventListener('DOMContentLoaded', () => {
    ...
    initGame();
    function handleCardClick(e) {
        if (gameState.isPaused || gameState.isProcessing || gameState.timeElapsed === 0) return;
        
        const card = e.currentTarget;
        if (card === gameState.firstCard || 
    ...
    function handleMismatch() {
        gameState.firstCard.classList.add('mismatch');
        gameState.secondCard.classList.add('mismatch');
        
        setTimeout(() => {
            gameState.firstCard.classList.remove('flipped', 'mismatch');
            gameState.secondCard.classList.remove('flipped', 'mismatch');
            resetTurn();
        }, 500);
    }
